...
if (isset($_REQUEST["user_id"])) {
    ...
    $sql = "SELECT tbl.* FROM (SELECT p.*,u.user_name,u.photo_link,c.category_title,c.category_icon,c.category_color
...
FROM posts p,users u,categories c WHERE p.category_id IN ($categories)  AND u.user_id=p.user_id AND p.category_id=c.category_id ORDER BY $sorting $asc $limit) AS tbl INNER JOIN likes ON tbl.post_id=likes.post_id WHERE likes.user_id=$userid";
