...
if (isset($_REQUEST["categories"])) {
    ...
    if(isset($_REQUEST["search"])){
		...
		$str= str_replace(" ", "%", $_REQUEST["search"]);
        $search="AND p.post_title LIKE '%".$str."%'";
    }
...
FROM posts p,users u,categories c WHERE p.category_id IN ($categories) AND u.user_id=p.user_id AND p.category_id=c.category_id $search ORDER BY $sorting $asc $limit";
